...
func WriteConfigStatus(w io.Writer, format Format, status config.Status) error {
	...
	case FormatCSV:
		...
		if err := writer.Write([]string{
			"config_file", "exists", "schema_version", "source_schema_version", "grant", "place", "sell", "cancel", "amend", "allow_live_order_actions", "complete_trade_auth", "accept_product_ack", "accept_fx_consent", "legacy_fields",
		}); err != nil {
			return err
		}
		if err := writer.Write([]string{
